...
- Material de cátedra (Google Slides). *HTTP Protocol*. Presentación de cátedra: formato de request/response, HTTP Headers por categoría, Request/Response Body (single/multi-resource), verbos HTTP, status codes, safe e idempotencia. Consultada mediante el conector Google Drive; se usó únicamente la porción correspondiente a Protocolo HTTP Parte 2 (el resto del material, sobre características del protocolo, historia y conexiones, corresponde a Protocolo HTTP Parte 1).
