...
Conviene precisar qué significan "cliente" y "servidor" en el contexto de HTTP, porque son roles del protocolo, no tipos fijos de programa. Un navegador web es el ejemplo más habitual de cliente HTTP, pero cualquier programa que envíe un request y espere un response cumple ese rol: un script de línea de comandos, una aplicación móvil que consume una API, o incluso otro servidor que a su vez actúa como cliente de un tercer servicio (un patrón habitual en arquitecturas donde un servidor intermedio agrega datos de varias fuentes antes de responder a su propio cliente). De manera análoga, un servidor HTTP puede ser un servidor de archivos estáticos, una aplicación que genera contenido dinámicamente, o un proxy que reenvía la request hacia otro servidor.

Esta flexibilidad de roles introduce un matiz relevante: los mismos principios de HTTP se aplican tanto si el "servidor" es una única máquina física dedicada, como si en realidad es una capa de proxies y múltiples instancias de aplicación, todas presentándose ante el cliente como un único punto de contacto identificado por un nombre de host. Desde la perspectiva del protocolo, esa complejidad interna es irrelevante: el cliente solo necesita saber que, al enviar una request a una dirección determinada, va a recibir una response coherente, sin que le importe cuántos componentes internos colaboraron para producirla.
...
1. El cliente inicia una request HTTP estableciendo una conexión TCP hacia un puerto determinado en el servidor (el three-way handshake recién descrito).
...
Una limitación estructural del modelo cliente-servidor es que el servidor no puede enviar datos sin que el cliente haya iniciado una request. Como workaround, las aplicaciones web recurren a: pings periódicos mediante `fetch`, WebSockets (canal bidireccional persistente), o Server-Sent Events (servidor mantiene abierto un response enviando actualizaciones sobre una conexión HTTP de larga duración).

```javascript
// Ejemplo conceptual: una request HTTP simple usando fetch (API del navegador).
...
En **HTTP/1.1** se permiten conexiones persistentes, reutilizando una única conexión TCP para varios pares request/response. Sin embargo, las requests deben procesarse en orden: una response lenta bloquea las siguientes, aun cuando estén listas. Los navegadores lo resuelven abriendo múltiples conexiones en paralelo hacia cada dominio (típicamente 6 conexiones actualmente).
...
**HTTP/2** resuelve este problema introduciendo la **multiplexación**: entrelaza múltiples mensajes sobre una única conexión, de modo que una response lenta no bloquea las demás, eliminando la necesidad de conexiones paralelas.
...
Esta persistencia tiene un costo que hay que administrar explícitamente: cada conexión abierta consume un socket TCP y memoria en el servidor durante todo el tiempo que dura la sesión, a diferencia de una request HTTP corriente que libera sus recursos apenas se envía el response. Escalar un servidor con miles de conexiones WebSocket simultáneas exige el mismo tipo de arquitectura non-blocking (event loop, event demultiplexing) discutida para servidores web en general, aplicada ahora a conexiones de larga duración en lugar de requests breves.
...
```javascript
// Ejemplo en Node.js: leer un body JSON desde una request HTTP entrante.
...
- Material de cátedra (Google Slides). *HTTP Protocol*. Presentación introductoria del módulo: características del protocolo, HTTP Session, HTTP y Hypertext, estableciendo la conexión, conexiones persistentes y domain sharding. Consultada mediante el conector Google Drive; se usó únicamente la porción correspondiente a Protocolo HTTP Parte 1 (el resto del material, sobre mensajes, headers, métodos y status codes, corresponde a Protocolo HTTP Parte 2).
